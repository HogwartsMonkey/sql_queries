...
DECLARE @Selected DATE = DATEADD(DAY,-1,getdate())
...
Declare @ThisWeek TABLE(
		...
		Retention_Purchasers FLOAT,
		Retention_Purchases FLOAT,
...
SELECT
	...
	Retention_Purchasers.retention_purchasers AS 'Retention_Purchasers',
	Retention_Purchases.retention_purchases AS 'Retention_Purchases',
...
WHERE DATEDIFF(month,[Day],GETDATE())<=4 
...
--retention purchases--
	LEFT JOIN (SELECT
		transaction_date,
        COUNT(B.userid) AS retention_purchases
		...
		FROM
			...
			(SELECT
				...
				amount,
				transaction_ranked,
                transaction_ranked_for_each_month,
				COUNT(*) AS duplicate
               --DATEDIFF(month,LAG(transaction_date,1) OVER (PARTITION BY userid ORDER BY transaction_date ASC),transaction_date) AS month_diff_perv_transaction
				...
				FROM
					...
					(SELECT 
								...
								SUM(amount) AS amount,
								RANK() OVER (PARTITION BY userid ORDER BY transaction_dt ASC) AS transaction_ranked,
                                RANK() OVER (PARTITION BY userid,YEAR(transaction_dt),MONTH(transaction_dt) ORDER BY transaction_dt ASC) AS transaction_ranked_for_each_month
            ...
            WHERE transaction_ranked !=1
            AND transaction_ranked_for_each_month >= 1
           --AND month_diff_perv_transaction =0

            GROUP BY transaction_date) AS Retention_Purchases

	ON performance_by_date.Day = Retention_Purchases.transaction_date

--end of rentention purchases--

--rentention purchasers--
	LEFT JOIN (SELECT
		transaction_date,
        COUNT(B.userid) AS retention_purchasers
		...
		FROM
           ...
           --AND month_diff_perv_transaction =0
            ...
            GROUP BY transaction_date) AS Retention_Purchasers

	ON performance_by_date.Day = Retention_Purchasers.transaction_date

--end of rentetion purchasers --
...
INSERT INTO @ThisWeek (_year,_month,ranking,cost,impressions,clicks,signups,full_signups,Same_Month_FTD,Previous_Months_FTD,Retention_Purchasers,Retention_Purchases,first_deposit_same_month,
		...
		SELECT
				...
				SUM(retention_purchasers),
				SUM(retention_purchases),
...
SELECT 
	...
	Same_Month_FTD,
	Previous_Months_FTD,
	Retention_Purchasers AS 'Retention_Purchasers(unique_user)',
	Retention_Purchases AS 'Retention_Purchases',
	...
	(first_deposit_same_month+first_deposit_previous_month) AS 'TotalFirstPurchases',
	(new_deposit_same_month+new_deposit_previous_month) AS 'RetentionPurchases',
	(first_deposit_same_month+first_deposit_previous_month+new_deposit_same_month+new_deposit_previous_month) AS 'TotalPurchases'
