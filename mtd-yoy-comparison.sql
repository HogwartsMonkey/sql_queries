		...
		FROM
			...
			(SELECT
				...
				FROM
					...
					GROUP BY transaction_dt,userid) AS transaction_ranked_and_grouped_by_date_and_userid
				...
				transaction_ranked,
				month,
				year) AS transactions_by_user_ranked
			...
			ON transactions_by_user_ranked.userid = month_of_reg.userid


		WHERE transaction_ranked= 1
		AND transactions_by_user_ranked.month != month_of_reg.month
		AND transactions_by_user_ranked.year >= month_of_reg.year
...
/*>= 2nd deposit && transaction month == reg month */
...
INSERT INTO @MTD_YoY (_year,_month,ranking,cost,impressions,clicks,signups,full_signups,Same_Month_FTD,Previous_Months_FTD,first_deposit_same_month,
...
SELECT 
 ...
 FROM @MTD_YoY  ORDER BY _year,_month ASC